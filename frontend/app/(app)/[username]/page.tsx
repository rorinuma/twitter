    ...
    !isLoading && (
      <div className="flex flex-col relative w-full h-full">
        ...
        <div className="flex justify-between p-2 mx-2 h-[100px]">
          ...
          {user?.id === owner.id ? (
            <button className="flex h-fit border py-2 px-4 font-bold border-border-muted rounded-full hover:bg-nav-hover duration-(--hover-duration)">
              Edit profile
            </button>
          ) : (
        ...
        {!postsLoading &&
          tweets.map((t) => (
            <TweetCard tweet={t} key={t.id} variant="default" />
          ))}
        {hasNextPage && (
